...
void driver(string inFile)
{
    ...
    thread aSortThread(
        [&] {
            bubble_sort(structArr, markers[97]);
        }
    );

    thread bSortThread(
        [&] {
            bubble_sort(structArr, markers[98]);
        }
    );

    thread cSortThread(
        [&] {
            bubble_sort(structArr, markers[99]);
        }
    );

    thread dSortThread(
        [&] {
            bubble_sort(structArr, markers[100]);
        }
    );

    thread eSortThread(
        [&] {
            bubble_sort(structArr, markers[101]);
        }
    );

    thread fSortThread(
        [&] {
            bubble_sort(structArr, markers[102]);
        }
    );

    thread gSortThread(
        [&] {
            bubble_sort(structArr, markers[103]);
        }
    );

    thread hSortThread(
        [&] {
            bubble_sort(structArr, markers[104]);
        }
    );

    thread iSortThread(
        [&] {
            bubble_sort(structArr, markers[105]);
        }
    );

    thread jSortThread(
        [&] {
            bubble_sort(structArr, markers[106]);
        }
    );

    thread kSortThread(
        [&] {
            bubble_sort(structArr, markers[107]);
        }
    );

    thread lSortThread(
        [&] {
            bubble_sort(structArr, markers[108]);
        }
    );

    thread mSortThread(
        [&] {
            bubble_sort(structArr, markers[109]);
        }
    );

    thread nSortThread(
        [&] {
            bubble_sort(structArr, markers[110]);
        }
    );

    thread oSortThread(
        [&] {
            bubble_sort(structArr, markers[111]);
        }
    );

    thread pSortThread(
        [&] {
            bubble_sort(structArr, markers[112]);
        }
    );

    thread qSortThread(
        [&] {
            bubble_sort(structArr, markers[113]);
        }
    );

    thread rSortThread(
        [&] {
            bubble_sort(structArr, markers[114]);
        }
    );

    thread sSortThread(
        [&] {
            bubble_sort(structArr, markers[115]);
        }
    );

    thread tSortThread(
        [&] {
            bubble_sort(structArr, markers[116]);
        }
    );

    thread uSortThread(
        [&] {
            bubble_sort(structArr, markers[117]);
        }
    );

    thread vSortThread(
        [&] {
            bubble_sort(structArr, markers[118]);
        }
    );

    thread wSortThread(
        [&] {
            bubble_sort(structArr, markers[119]);
        }
    );

    thread xSortThread(
        [&] {
            bubble_sort(structArr, markers[120]);
        }
    );

    thread ySortThread(
        [&] {
            bubble_sort(structArr, markers[121]);
        }
    );

    thread zSortThread(
        [&] {
            bubble_sort(structArr, markers[122]);
        }
    );
